...
func ContentDoctorToPatientHomecare(content string, homecare model.Homecare) string {
	...
	replacer := strings.NewReplacer("@doctor", homecare.Chat.Doctor.Name, "@visit_at", homecare.VisitAt.In(timeLocation).Format("02-01-2006"), "@hour", homecare.VisitAt.In(timeLocation).Format("15:04"), "@patient", homecare.Chat.User.Name)
	content = replacer.Replace(content)
	return content
}
